...
def encode_spec_reg(i_model): 
    ''' encode SDSS spec using trained regression compression  
    '''
    device = torch.device(type='cuda', index=0)
    ...
    best_model = torch.load(os.path.join(
        ...
        f"regressor_compressor.{i_model}.pt"))
    best_model.to(device)
    ...
    for batch in loader: 
        ...
        h = best_model.forward(spec, w=w, z=z)
        h_spec.append(h.detach().cpu())

    h_spec = np.concatenate(h_spec, axis=0)
    np.save('/scratch/network/chhahn/sedflow/sdss.clean.h_spec.reg_comp.npy', h_spec)
    return None 


def encode_spec_reg_test(i_model): 
    ''' encode SDSS spec using trained regression compression  
    '''
    device = torch.device(type='cuda', index=0)
    ...
    # load SDSS data
    wave    = np.load('/scratch/network/chhahn/sedflow/sdss.clean.wave.npy')
    spec    = np.load('/scratch/network/chhahn/sedflow/sdss.test.spec.npy')
    norm    = np.load('/scratch/network/chhahn/sedflow/sdss.test.A_spec.npy')
    wght    = np.load('/scratch/network/chhahn/sedflow/sdss.test.wght.npy')
    zred    = np.load('/scratch/network/chhahn/sedflow/sdss.test.zred.npy')

    spec_w  = spec/norm[:,None]
    ...
    h_spec = np.concatenate(h_spec, axis=0)
    np.save('/scratch/network/chhahn/sedflow/sdss.test.h_spec.reg_comp.npy', h_spec)
    return None 
...
if __name__=='__main__': 
    ...
    if task == 'clean_data': 
        ...
        train_nde_noise()
    elif task in ['encode_spec_reg', 'encode_spec_reg_test']: 
        sys.path.append('/home/chhahn/projects/SEDflow/docs/nb/spectra/spectrum-encoder/')
        ...
        if task == 'encode_spec_reg': 
            encode_spec_reg(i_model)
        elif task == 'encode_spec_reg_test': 
            encode_spec_reg_test(i_model)
