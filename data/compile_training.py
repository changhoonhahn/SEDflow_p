...
for ibatch in range(10): 
    ...
    f_hspec = os.path.join(dat_dir, 'train.v0.1.%i.h_spec.reg_comp.nde_noise.npy' % ibatch) 
    ...
    _Aspec = np.load(f_Aspec)
    zero_norm = (_Aspec == 0)

    theta_sps.append(np.load(fsps)[~zero_norm])
    theta_unt.append(np.load(funt)[~zero_norm])

    A_spec.append(np.load(f_Aspec)[~zero_norm])
    h_spec.append(np.load(f_hspec))
    A_ivar.append(np.load(f_Aivar)[~zero_norm])
    h_ivar.append(np.load(f_hivar)[~zero_norm])

    zred.append(np.load(f_zred)[~zero_norm])
...
np.save('/scratch/network/chhahn/sedflow/sedflow_p.train.v0.2.theta_sps.npy',
        theta_sps[not_nan]) 
np.save('/scratch/network/chhahn/sedflow/sedflow_p.train.v0.2.theta_unt.npy', 
        theta_unt[not_nan]) 
np.save('/scratch/network/chhahn/sedflow/sedflow_p.train.v0.2.norm_spec.npy', 
        A_spec[not_nan]) 
np.save('/scratch/network/chhahn/sedflow/sedflow_p.train.v0.2.h_spec.npy', 
        h_spec[not_nan]) 
np.save('/scratch/network/chhahn/sedflow/sedflow_p.train.v0.2.norm_ivar.npy', 
        A_ivar[not_nan]) 
np.save('/scratch/network/chhahn/sedflow/sedflow_p.train.v0.2.h_ivar.npy', 
        h_ivar[not_nan]) 
np.save('/scratch/network/chhahn/sedflow/sedflow_p.train.v0.2.zred.npy', 
        zred[not_nan]) 
