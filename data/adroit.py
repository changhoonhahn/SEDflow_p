...
def encode_spec_reg_sdss(i_model):
    '''
    '''
    cntnt = '\n'.join([
        "#!/bin/bash", 
        "#SBATCH -J sdss.enc_spec.reg.%i" % (i_model), 
        "#SBATCH --partition=general",
        "#SBATCH --time=02:59:59", 
        "#SBATCH --export=ALL", 
        "#SBATCH --gres=gpu:1", 
        "#SBATCH --output=o/sdss.enc_spec.reg.%i.o" % (i_model), 
        "#SBATCH --mail-type=all", 
        "#SBATCH --mail-user=chhahn@princeton.edu", 
        "", 
        "", 
        ...
        "conda activate torch-env", 
        "",
        "python /home/chhahn/projects/SEDflow/bin/sedflow_p/data/process_sdss.py encode_spec_reg %i" % (i_model), 
        "",
        'now=$(date +"%T")', 
        'echo "end time ... $now"', 
        ""]) 
...
def encode_spec_reg_sdss_test(i_model):
    '''
    '''
    cntnt = '\n'.join([
        "#!/bin/bash", 
        "#SBATCH -J sdss.enc_spec.test.reg.%i" % (i_model), 
        "#SBATCH --partition=general",
        "#SBATCH --time=02:59:59", 
        "#SBATCH --export=ALL", 
        "#SBATCH --gres=gpu:1", 
        "#SBATCH --output=o/sdss.enc_spec.test.reg.%i.o" % (i_model), 
        "#SBATCH --mail-type=all", 
        "#SBATCH --mail-user=chhahn@princeton.edu", 
        "", 
        "", 
        ...
        "conda activate torch-env", 
        "",
        "python /home/chhahn/projects/SEDflow/bin/sedflow_p/data/process_sdss.py encode_spec_reg_test %i" % (i_model), 
        "",
        'now=$(date +"%T")', 
        'echo "end time ... $now"', 
        ""]) 
...
def apply_nde_noise(ibatch, n_latent, i_model): 
    ...
    cntnt = '\n'.join([
        ...
        "conda activate torch-env", 
        "",
        "python /home/chhahn/projects/sedflow_p/data/process_train.py apply_nde_noise %i %i %i" % (ibatch, n_latent, i_model),
        "",
        'now=$(date +"%T")', 
        'echo "end time ... $now"', 
        ""]) 
...
def encode_spec_reg_train(ibatch, i_model): 
    ''' encode noisy training spectra
    '''
    cntnt = '\n'.join([
        "#!/bin/bash", 
        "#SBATCH -J enc_spec.reg.train%i" % (ibatch), 
        "#SBATCH --partition=general",
        "#SBATCH --time=01:59:59", 
        "#SBATCH --mem-per-cpu=16G", 
        "#SBATCH --export=ALL", 
        "#SBATCH --gres=gpu:1", 
        "#SBATCH --output=o/enc_spec.reg.train%i.o" % (ibatch), 
        "#SBATCH --mail-type=all", 
        "#SBATCH --mail-user=chhahn@princeton.edu", 
        "", 
        'now=$(date +"%T")', 
        ...
        "conda activate torch-env", 
        "",
        "python /home/chhahn/projects/SEDflow/bin/sedflow_p/data/process_train.py encode_spec_reg  %i %i" % (ibatch, i_model),
        "",
        'now=$(date +"%T")', 
        'echo "end time ... $now"', 
        ""]) 
...
# encoded SDSS spectra using regression compressor 
#encode_spec_reg_sdss(2)
#encode_spec_reg_sdss_test(2)
...
#for i in range(10): 
#    encode_spec_reg_train(i, 2)
#encode_spec_reg_train(101, 2)
