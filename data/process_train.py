...
def apply_nde_noise(ibatch, n_latent, i_model): 
    ...
    # calculate ivar 
    ivar = ((w_ivar * std_ivar + avg_ivar) * Ah_ivar[:,0][:,None]).clip(1e-5, None)
...
def encode_spec_regression(ibatch, i_model): 
    ...
    train_ivar = np.load(os.path.join(dat_dir, 
        f'train.v0.1.{ibatch}.ivar.nde_noise.npy'))
    train_norm = np.load(os.path.join(dat_dir, 
        f'train.v0.1.{ibatch}.norm_spec.nde_noise.npy'))
    train_zred = np.load(os.path.join(dat_dir, 
        f'train.v0.1.{ibatch}.redshifts.npy'))

    zero_norm = (train_norm == 0)
    train_seds  = train_seds[~zero_norm]
    ...
    np.save(os.path.join(dat_dir, f'train.v0.1.{ibatch}.h_spec.reg_comp.nde_noise.npy'), 
            h_train)
    return None 
...
if __name__=="__main__":
    ...
    if task == 'sample_nde_noise': 
        ...
        encode_spec_regression(ibatch, i_model)
