...
def mcmc_mock(i0, i1, niter=5000):
    ''' deploy ANPE training 
    '''
    cntnt = '\n'.join([
        "#!/bin/bash", 
        "#SBATCH -J mcmc.mock.%i_%i" % (i0, i1), 
        "#SBATCH --partition=general",
        "#SBATCH --time=11:59:59", 
        "#SBATCH --export=ALL", 
        "#SBATCH --output=o/mcmc.mock.%i_%i.o" % (i0, i1), 
        "#SBATCH --mail-type=all", 
        "#SBATCH --mail-user=chhahn@princeton.edu", 
        "", 
        'now=$(date +"%T")', 
        ...
        "conda activate torch-env", 
        "",
        "python /home/chhahn/projects/SEDflow_p/mcmc/mcmc_train.py %i %i %i" % (niter, i0, i1), 
        "",
        'now=$(date +"%T")', 
        'echo "end time ... $now"', 
        ""]) 
...
# run mcmc on mock observations
mcmc_mock(0, 9)
for i in range(1, 10): 
    mcmc_mock(10*i, 10*(i+1)-1)
